...
using static Extensions.Lists.NeighbourExtensions;
...
namespace Extensions.Lists
{
    public static class InterpolationExtensions
    {
        ...
        {
            return list.ToList().Interpolate(p => (int)p.ToNonNullable().WeightedAverage(), p => p == null);
        }
        public static IEnumerable<double?> Interpolate(this IEnumerable<double?> list)
        {
            return list.ToList().Interpolate(p => p.ToNonNullable().WeightedAverage(), p => p == null);
        }

        public static IEnumerable<string> Interpolate(this IEnumerable<string> list)
        {
            return list.ToList().Interpolate(p => p.Nearest(), p => p == null);
        }
        ...
        {
            return list.ToList().Interpolate(interpolation, needsToBeInterpolated);
        }
